...
import { MapViewBase, BoundsBase, CircleBase, MarkerBase, PolygonBase, PolylineBase, ProjectionBase, PositionBase, ShapeBase, latitudeProperty, VisibleRegionBase, longitudeProperty, bearingProperty, zoomProperty, tiltProperty, UISettingsBase, getColorHue } from './common';
...
import { CameraUpdateFactory, ClusterManager, CustomClusterItem, CustomClusterManager, CustomClusterRenderer, LatLngBounds } from './clustering/clusterer.android';
...
export class MapView extends MapViewBase {
	...
	public createNativeView() {
		...
		var mapReadyCallback = new com.google.android.gms.maps.OnMapReadyCallback({
			onMapReady: (gMap) => {
				...
				gMap.setOnInfoWindowCloseListener(
				...
				gMap.setOnIndoorStateChangeListener(
				...
				if (gMap.setOnCameraIdleListener) {
					...
					gMap.setOnCameraChangeListener(
						new com.google.android.gms.maps.GoogleMap.OnCameraChangeListener({
	...
	public setupMarkerCluster(markers: Marker[], minClusterNumber: number) {
		...
		var maxZoomLevel: number;

		clusterManager.setRenderer(renderer);

		clusterManager.mapView = _this;

		if (_this.gMap.setOnCameraIdleListener) {
			_this.gMap.setOnCameraIdleListener(clusterManager);
		} else if (_this.gMap.setOnCameraChangeListener) {
			_this.gMap.setOnCameraChangeListener(clusterManager);
		}

		_this.gMap.setOnMarkerClickListener(clusterManager);
		_this.gMap.setOnInfoWindowClickListener(clusterManager);

		clusterManager.setOnClusterItemClickListener(
			new ClusterManager.OnClusterItemClickListener({
				onClusterItemClick: function (item) {
					_this.findMarker((marker) => {
						if (marker.position.latitude === item.getPosition().latitude && marker.position.longitude === item.getPosition().longitude) {
							_this.notifyMarkerEvent('markerSelect', marker);
							return true;
						} else {
							return false;
						}
		...
		clusterManager.setOnClusterClickListener(
			new ClusterManager.OnClusterClickListener({
				onClusterClick: function (cluster) {
					var listeMarker = cluster.getItems().toArray();
					// Cluster zoom in logic
					var builder = LatLngBounds.builder();
					for (var i = 0; i < listeMarker.length; i++) {
						builder.include(listeMarker[i].getPosition());
					}
					var bounds = builder.build();

					_this.gMap.animateCamera(CameraUpdateFactory.newLatLngBounds(bounds, 100));
	...
	setStyle(style: MapStyle): boolean {
		if (!this.gMap) return null;
